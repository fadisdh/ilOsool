...
@section('sidemenu')
	@include('request.filter_requests')
@stop
...
@section('user_content')
	...
	<div class="page-content">
		<div class="row clearfix">
			<div class="col-md-9">
				<h2 class="page-title">{{trans('request.requested_deals')}}</h2>
			</div>
			<div class="col-md-3 addcompany-btn">
				<a href="{{ URL::route('request.deal') }}" class="btn btn-primary" title="{{trans('request.add_request')}}">{{trans('request.add_request')}}</a>
			</div>
		</div>
		...
		<ul class="list-unstyled mycompanies companies-list clearfix">
				...
				@foreach($requests as $request)
					<li class="row company-item animation">
						<div class="company-item-block">
							<a href="{{URL::route('request.view', $request->id)}}" class="company-item-row row">
							<div class="col-md-5">
								<div class="row">
			...
			@else
				<div class="no-result">
					<h3>{{trans('request.no_requests')}}</h3>
				</div>
			@endif
		</ul>
